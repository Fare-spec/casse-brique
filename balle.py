...
class Balle:
	...
	def doitRebondir(self, raquette):
		return self.xpos >= raquette.xpos and \
				self.xpos <= raquette.xpos + raquette.largeur and \
				self.ypos >= HAUTEUR_ECRAN-30

	def deplace(self, raquette):
		self.xpos += self.xvit
		self.ypos += self.yvit
		if self.doitRebondir(raquette):
			angle_degre = 10 + 160*(1-(self.xpos - raquette.xpos)/raquette.largeur)
